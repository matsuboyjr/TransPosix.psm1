...
foreach($name in @('ls','cat','cp','mv','rm','sort')){$alias=Get-Alias $name -ErrorAction SilentlyContinue;if($alias){$script:OriginalAliases[$name]=[pscustomobject]@{Definition=$alias.Definition;Description=$alias.Description;Options=$alias.Options}}}
$definitions = & $parentModule {
    @{
        TransPosixLsCommand=${function:Invoke-TransPosixLsDirect};TransPosixCatCommand=${function:Invoke-TransPosixCatDirect}
...
Export-ModuleMember -Function TransPosixLsCommand,TransPosixCatCommand,TransPosixCpCommand,TransPosixMvCommand,TransPosixRmCommand,mkdir,TransPosixSortCommand,date
